    ...
    <div className="bg-white p-6 rounded-lg shadow-md min-h-screen">
      ...
      <div id="printable-area">
          ...
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-gray-800">Subject Performance Leaderboard of {filters.gradeLevel}</h2>
          ...
          {/* DATA TABLE (Visible when printing) */}
          {data.length > 0 ? (
                  ...
                  {data.map((subject, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap font-medium text-gray-900 border-r">
                        {subject.subjectName}
                        <span className="text-gray-500 text-xs ml-1 font-bold">({subject.totalPossibleScore})</span>
                      </td>
                      <td className={`px-2 py-3 text-center font-bold border-r ${subject.averageScore < 50 ? 'text-red-600' : subject.averageScore > 80 ? 'text-green-600' : 'text-blue-600'}`}>
                        {subject.averageScore}
                      </td>
                      <td className="px-2 py-3 text-center border-r">
                        <span className={`px-2 py-1 rounded text-xs font-bold ${parseFloat(subject.passRate) < 50 ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
                          {subject.passRate}
                        </span>
                      </td>
                      <td className={`px-2 py-3 text-center text-sm border-r ${subject.ranges.below50 > 0 ? 'bg-red-50 text-red-700 font-bold' : 'text-gray-300'}`}>{subject.ranges.below50}</td>
                      <td className={`px-2 py-3 text-center text-sm border-r ${subject.ranges.below75 > 0 ? 'bg-yellow-50 text-yellow-800' : 'text-gray-300'}`}>{subject.ranges.below75}</td>
                      <td className={`px-2 py-3 text-center text-sm border-r ${subject.ranges.below90 > 0 ? 'bg-blue-50 text-blue-800' : 'text-gray-300'}`}>{subject.ranges.below90}</td>
                      <td className={`px-2 py-3 text-center text-sm ${subject.ranges.above90 > 0 ? 'bg-green-50 text-green-800 font-bold' : 'text-gray-300'}`}>{subject.ranges.above90}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
