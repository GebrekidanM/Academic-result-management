...
const QuizCenter = ({ quizzes, quizStatuses }) => {
  ...
  useEffect(() => {
    const syncServerTime = async () => {
      try {
        ...
        console.warn("Time sync failed or endpoint not found, falling back to local time.");
  ...
  return (
    <div className="space-y-8">
      {/* HEADER */}
      <div>
        <h2 className="text-2xl font-black text-slate-800">Quiz Center</h2>
      ...
      <section className="space-y-5">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-black text-slate-700">Pending Quizzes</h3>
        ...
        {pendingQuizzes.length === 0 ? (
          <div className="bg-white border border-slate-100 rounded-2xl shadow-sm p-10 text-center">
            <p className="text-slate-500">No pending quizzes available.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {pendingQuizzes.map((quiz) => (
              <Quiz
                ...
                serverTimeOffset={serverTimeOffset} // Passed from Parent
              />
            ))}
          </div>
        )}
      ...
      {completedQuizzes.length > 0 && (
        <section className="space-y-4">
          <button
            ...
            className="
              w-full bg-white border border-slate-100 rounded-2xl shadow-sm px-6 py-4 
              flex items-center justify-between font-black text-slate-700 
            ...
            <div className="space-y-3 animate-fadeIn">
              {completedQuizzes.map((quiz) => (
                <div
                  key={quiz._id}
                  className="
                    bg-white border border-slate-100 rounded-2xl shadow-sm px-6 py-4 
                ...
                >
                  ...
                  <button
                    ...
                    className="
                      bg-pink-600 hover:bg-pink-700 text-white px-4 py-2 
