...
const SubjectCard = ({ subject }) => {
  // ⚠️ ማስተካከያ፦ ወራቶቹን በጊዜ ቅደም ተከተል (ከመስከረም እስከ ሰኔ) ለመደርደር የተጨመረ useMemo [2]
  const sortedGroupedByMonth = useMemo(() => {
    if (!subject?.groupedByMonth) return [];
    return Object.entries(subject.groupedByMonth).sort(
      ([monthA], [monthB]) => MONTHS_ORDER.indexOf(monthA) - MONTHS_ORDER.indexOf(monthB)
    );
  }, [subject]);

  return (
    <div className="bg-white hover:bg-slate-50 border border-slate-100 rounded-2xl shadow-sm overflow-hidden transition-all duration-200">
      {/* HEADER */}
      ...
      <div className="p-6 space-y-6">
        {sortedGroupedByMonth.map(([month, assessments]) => (
          <div key={month}>
            {/* MONTH */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {assessments.map((assessment) => (
                <div key={assessment.id} className="bg-slate-55 border border-slate-100 rounded-xl p-4 flex items-center justify-between">
                  <div>
                    <p className="font-bold text-slate-700"> {typeof assessment.name === 'object' ? assessment.name?.name : assessment.name}</p>
