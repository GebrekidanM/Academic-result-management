  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // PARENT LOGIN
        ...
        if (response.data.token) {
          await AsyncStorage.setItem("student-user", JSON.stringify(response.data));
        ...
        <View className="flex-1 bg-gray-100 px-6 py-10">
          ...
          <View className="flex-1 justify-center">
            <View className="bg-white rounded-3xl p-6 shadow">
                <Text className="text-3xl font-bold text-center text-pink-600"> Welcome </Text>
                <Text className="text-center text-cyan-700 mt-2 mb-8"> Please login to continue </Text>

                {/* USERNAME */}
                <Text className="mb-2 text-cyan-800 font-semibold"> Username</Text>
                <TextInput
                  placeholder="Enter username"
                  className="bg-gray-100 text-cyan-900 rounded-xl p-4 mb-4"
                  value={formData.username}
                  onChangeText={(text) => setFormData({...formData, username: text })}
                />

                {/* PASSWORD */}
                <Text className="mb-2 text-cyan-800 font-semibold">Password</Text>
                <TextInput
                ...
                {/* ROLE */}
                <Text className="mb-2 text-cyan-800 font-semibold">Select Role</Text>
                <View className="flex-row gap-3 mb-6">
                <TouchableOpacity
                    className={`flex-1 p-4 rounded-xl items-center ${
                    formData.role === "staff" ? "bg-pink-600" : "bg-gray-200"}`}
